...
- Recording encoding (minimal size): `start_recording` uses RoomCompositeEgress with `single-speaker` layout + `advanced` EncodingOptions — 1280x720@30, `videoBitrate: 1000`, `audioBitrate: 64` (OPUS/H264_MAIN egress defaults) ≈ 0.47 GB/h vs the default preset's ≈ 1.4 GB/h. Session format: the mentor is the only video publisher and students are audio-only, so the composite always shows the mentor and student questions land in the mixed audio (verified against `livekit/egress` `template-default`: the single-speaker template renders video publications only — audio-only participants never get tiles). Caveats: with `advanced`, unset fields fall back to 1080p/4500k so set width/height/framerate/bitrates explicitly; SDK 2.1.2 presets have no 360P/480P members (lower presets were removed/renumbered in the protocol — never hardcode preset ints); MP4+OPUS won't play in pre-17 Safari (in-app Video.js playback is fine on modern browsers; switch `audioCodec` to `COMPATIBLE_AAC` if older-Safari downloads matter).
- Recording UI (MeetingView): host-only `Record` toggle in both control bars (`ControlActionButton` now supports `disabled`) — gate is `isHost` (mentor OR admin co-host), matching the edge function's mentor-or-admin authorization. State restores from `session_recordings` on refresh (`.maybeSingle()` over `starting|recording`; hosts only — students have no SELECT by RLS). `handleToggleRecording` wraps the fetch in try/catch/finally with string-coerced error toasts; on success it broadcasts `RECORDING_STARTED/STOPPED` on the data channel and mirrors `isRecording` in the host's participant metadata. Participants get a REC chip in the stage: instant via the data channel (sender-validated — only mentor identity or an admin co-host may move the chip, same predicate as the poll's host scan, so forged broadcasts can't hide an active recording), and re-synced for late joiners every 3s by the metadata poll (host identities only: `identity === mentorId` or admin). Recording errors surface as toasts, never stuck spinners. Host clients (mentor + admin co-host) keep their toggle state in sync via `syncRecordingFromDb` — the active `session_recordings` row is the single source of truth; recording broadcasts act only as triggers (they are client-forgeable, the DB row is not), the poll-vs-local mismatch effect catches missed events after reconnects, and syncing after one's own toggle normalizes the timer against the server-stamped `started_at`; sync is skipped while a toggle is in flight (`isRecordingTogglingRef`).
