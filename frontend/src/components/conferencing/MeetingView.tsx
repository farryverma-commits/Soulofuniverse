...
export const MeetingView: React.FC<MeetingViewProps> = ({ token, serverUrl, sessionId, isMentor, mentorId, onDisconnected }) => {
  if (!token || !serverUrl) {
    return (
  ...
  return (
    <div className="h-screen h-[100dvh] bg-black overflow-hidden flex flex-col">
      ...
      >
        <MyVideoConference sessionId={sessionId} isMentor={isMentor} mentorId={mentorId} />
        <RoomAudioRenderer />
      </LiveKitRoom>
    </div>
  );
...
function MyVideoConference({ sessionId, isMentor, mentorId }: { sessionId: string; isMentor: boolean; mentorId: string }) {
  ...
  const sendChat = async (text: string) => {
    ...
    try {
      ...
      try {
        ...
        await localParticipant.publishData(data, { reliable: true, topic: 'lk-chat-topic' });
      } catch (manualErr) {
  ...
  useEffect(() => {
    console.log("Permissions updated:", { isMentor, canSpeak, isMutedByMentor, metadata: currentMetadata });
    ...
    checkMuteState();
  }, [isMentor, canSpeak, isMutedByMentor, currentMetadata, room, mentorId, localParticipant, sessionStatus, participants]);
    ...
    const onDataReceived = async (payload: Uint8Array, participant?: any) => {
      try {
        ...
        if (data.action === 'MUTE_ALL' && !isMentor) {
           ...
           const currentMeta = JSON.parse(localParticipant.metadata || '{}');
           await localParticipant.setMetadata(JSON.stringify({ ...currentMeta, isMutedByMentor: true }));
        }
        ...
        if (data.target === localParticipant?.identity) {
           ...
           if (data.action === 'ALLOW_SPEAK') {
             ...
             const newMetadata = { ...currentMeta, canSpeak: true, isMutedByMentor: false, handRaised: false, raisedAt: null };
             await localParticipant.setMetadata(JSON.stringify(newMetadata));
           } else if (data.action === 'REVOKE_SPEAK') {
             console.log("Received REVOKE_SPEAK command");
             const newMetadata = { ...currentMeta, canSpeak: false, isMutedByMentor: true };
             await localParticipant.setMetadata(JSON.stringify(newMetadata));
             await localParticipant.setMicrophoneEnabled(false);
             await localParticipant.setCameraEnabled(false);
           } else if (data.action === 'LOWER_HAND') {
             console.log("Received LOWER_HAND command");
             const newMetadata = { ...currentMeta, handRaised: false, raisedAt: null };
             await localParticipant.setMetadata(JSON.stringify(newMetadata));
           }
        }
      } catch(e) {
  ...
  const handleToggleRecording = async () => {
    ...
    if (isRecording) {
      await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/livekit-manage-session`, {
        ...
        body: JSON.stringify({ session_id: sessionId, action: 'stop_recording', egress_id: egressId }),
      });
      setIsRecording(false);
      setEgressId(null);
    } else {
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/livekit-manage-session`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${authSession?.access_token}` },
        body: JSON.stringify({ session_id: sessionId, action: 'start_recording' }),
      });
  ...
  const mainSpeakerTrack = useMemo(() => {
    ...
    console.log("Searching for mentor track. MentorId:", mentorId, "Tracks:", tracks.length);
    
    // Find track belonging to mentorId
    const mentorTrack = tracks.find(t => 
    ...
    if (isMentor) {
      const localCamera = tracks.find(t => 
    ...
    if (anyCamera) {
      console.log("Using secondary fallback camera:", anyCamera.participant.identity);
      return anyCamera;
    }
  ...
  const secondaryTracks = useMemo(() => {
    if (!mainSpeakerTrack) return tracks.filter(t => t.source === Track.Source.Camera);
  ...
  const otherParticipants = tracks.filter(t => 
    String(t.participant?.identity) !== String(mentorId) && 
    t.source !== Track.Source.ScreenShare
  );

  const activeSpeakers = otherParticipants.filter(t => 
    t.participant?.isSpeaking
  ).sort((a, b) => (b.participant?.lastSpokeAt || 0) - (a.participant?.lastSpokeAt || 0));
  ...
  const secondaryTrack = screenShareTrack ? mainSpeakerTrack : activeSpeakers[0];
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          ...
          <div className="absolute top-4 left-4 md:top-6 md:left-6 z-20 flex items-center gap-3">
             ...
             {isRecording && (
                <div className="bg-red-600/80 backdrop-blur-md px-3 py-1.5 md:px-4 md:py-2 rounded-full border border-red-400/20 flex items-center gap-2 shadow-2xl">
                   ...
                   <span className="text-[9px] md:text-[11px] font-black text-white uppercase tracking-[0.2em]">Rec</span>
                </div>
             )}
          </div>
          ...
          <div className="flex-1 overflow-hidden p-2 md:p-6 mb-24 md:mb-28">
            {tracks.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-gray-400">
                ...
                <p className="font-bold text-lg tracking-tight mt-4">Preparing your session...</p>
              </div>
            ) : layout === 'grid' ? (
              ...
              <div className="h-full w-full flex flex-col md:flex-row gap-4">
                ...
                <div className={`relative border border-white/10 rounded-2xl overflow-hidden bg-black/40 shadow-2xl transition-all duration-500 flex-[1.5]`}>
                  <FocusLayout trackRef={mainTrack}>
                    <ParticipantTile />
                  </FocusLayout>
                  <div className="absolute bottom-4 left-4 z-20 bg-black/40 backdrop-blur-md px-3 py-1 rounded-lg border border-white/10 text-[10px] font-bold uppercase tracking-widest">
                    {screenShareTrack ? 'Presentation' : (String(mainTrack?.participant?.identity) === String(mentorId) ? 'Mentor' : 'Speaker')}
                  </div>
                </div>
                  ...
                  <div className="flex-1 flex flex-col gap-4 animate-in fade-in slide-in-from-right-4 duration-500 min-h-0 overflow-y-auto">
                    {secondaryTrack && (
                      <div className="flex-1 relative border border-white/10 rounded-2xl overflow-hidden bg-black/40 shadow-2xl min-h-[200px]">
                        ...
                        <div className="absolute bottom-4 left-4 z-20 bg-black/40 backdrop-blur-md px-3 py-1 rounded-lg border border-white/10 text-[10px] font-bold uppercase tracking-widest">
                          {screenShareTrack && secondaryTrack?.participant?.identity === mentorId ? 'Mentor' : 'Active Speaker'}
                        </div>
                      </div>
                    )}
                    
                    {/* More Speakers (Carousel) if > 1 active speaker */}
                    {activeSpeakers.length > (secondaryTrack === mainSpeakerTrack ? 0 : 1) && (
                      <div className="h-32 md:h-48 shrink-0">
                        <CarouselLayout tracks={activeSpeakers.filter(t => t !== secondaryTrack)}>
                          <CustomParticipantTile />
                        </CarouselLayout>
                      </div>
                    )}
          ...
          {isMobile && !showSidebar ? (
            <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 bg-[#1A1A1A]/95 backdrop-blur-2xl border border-white/10 rounded-full px-5 py-3.5 shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex items-center gap-3.5 animate-in slide-in-from-bottom-8 duration-500">
              ...
              <ControlActionButton 
                onClick={toggleHand}
                icon={<Hand className={`w-5 h-5 ${isHandRaised ? 'text-yellow-400 fill-current' : 'text-white'}`} />}
                ...
                {showMoreMenu && (
                  <div className="absolute bottom-full right-0 mb-6 w-64 bg-[#1A1A1A]/95 backdrop-blur-2xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl z-[60] animate-in slide-in-from-bottom-4 duration-300">
                    <div className="px-5 py-4 border-b border-white/5 bg-white/5">
                      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Session Controls</span>
                    </div>
                    <div className="py-2">
                      <button 
                        onClick={() => { setSidebarTab('chat'); setShowSidebar(true); setShowMoreMenu(false); }}
                      ...
                      >
                        ...
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-gray-200">Chat</span>
                          <span className="text-[10px] text-gray-500">Send messages to all</span>
                        </div>
                      </button>
                      <button 
                        onClick={() => { setSidebarTab('participants'); setShowSidebar(true); setShowMoreMenu(false); }}
                        className="w-full px-5 py-4 text-left flex items-center gap-4 hover:bg-white/5 transition-colors group"
                      >
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] text-gray-500">Manage everyone</span>
                        </div>
                      </button>
                      <button 
                        onClick={() => { setLayout(layout === 'grid' ? 'speaker' : 'grid'); setShowMoreMenu(false); }}
                        className="w-full px-5 py-4 text-left flex items-center gap-4 hover:bg-white/5 transition-colors group"
                      >
                        <div className="w-8 h-8 rounded-lg bg-orange-500/10 flex items-center justify-center group-hover:bg-orange-500/20 transition-colors">
                          {layout === 'grid' ? <User className="w-4 h-4 text-orange-400" /> : <LayoutGrid className="w-4 h-4 text-orange-400" />}
                        </div>
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-gray-200">{layout === 'grid' ? 'Speaker View' : 'Grid View'}</span>
                          <span className="text-[10px] text-gray-500">Change perspective</span>
                        </div>
                      </button>
                      ...
                      {isMentor && (
                        <>
                          ...
                          <button 
                            ...
                            className="w-full px-5 py-4 text-left flex items-center gap-4 hover:bg-white/5 transition-colors group"
                          >
                            <div className={`w-8 h-8 rounded-lg ${localParticipant?.isScreenShareEnabled ? 'bg-green-500/20' : 'bg-gray-500/10'} flex items-center justify-center group-hover:opacity-80 transition-all`}>
                              <MonitorUp className={`w-4 h-4 ${localParticipant?.isScreenShareEnabled ? 'text-green-400' : 'text-gray-400'}`} />
                            </div>
                            <div className="flex flex-col">
                              <span className="text-sm font-bold text-gray-200">Share Screen</span>
                              <span className="text-[10px] text-gray-500">{localParticipant?.isScreenShareEnabled ? 'Currently presenting' : 'Start presenting'}</span>
                            </div>
                          </button>
                          <button 
                            onClick={() => { handleToggleRecording(); setShowMoreMenu(false); }}
                            className="w-full px-5 py-4 text-left flex items-center gap-4 hover:bg-white/5 transition-colors group"
                          >
                            <div className={`w-8 h-8 rounded-lg ${isRecording ? 'bg-red-500/20' : 'bg-gray-500/10'} flex items-center justify-center group-hover:opacity-80 transition-all`}>
                              <CircleDot className={`w-4 h-4 ${isRecording ? 'text-red-400 animate-pulse' : 'text-gray-400'}`} />
                            </div>
                            <div className="flex flex-col">
                              <span className="text-sm font-bold text-gray-200">Record Session</span>
                              <span className="text-[10px] text-gray-500">{isRecording ? "Stop cloud recording" : "Save this meeting"}</span>
                            </div>
                          </button>
                        </>
                      )}
               ...
               <div className="max-w-screen-2xl mx-auto flex items-center justify-between md:gap-4 overflow-x-auto no-scrollbar w-full">
                  {/* Left: Audio/Video Toggles */}
                   <div className="flex items-center gap-3 md:gap-4 shrink-0">
                      ...
                      <div className="relative group">
                        <div className={`relative flex items-center bg-white/10 rounded-2xl border border-white/10 transition-all overflow-hidden shadow-lg ${!canSpeak ? 'opacity-50 cursor-not-allowed' : 'hover:border-white/30'}`}>
                          <MediaControl 
                            source={Track.Source.Microphone}
                            disabled={!canSpeak}
                  ...
                  {/* Center: Main Actions */}
                   <div className="flex items-center gap-1 md:gap-2 shrink-0 px-2 md:px-0 border-x border-white/5 md:border-transparent mx-2 md:mx-0">
                     ...
                     {isMentor && (
                       <ControlActionButton 
                        ...
                        icon={<CircleDot className={`w-5 h-5 ${isRecording ? 'text-red-500 animate-pulse' : 'text-gray-100'}`} />}
                   ...
                   <div className="flex items-center gap-2 md:gap-4 shrink-0">
                     ...
                     >
                       <PhoneOff className="w-5 h-5 fill-current" />
                       <span className="hidden md:inline text-xs font-black uppercase tracking-widest">{isMentor ? 'End Session' : 'Leave'}</span>
                     </button>
                  </div>
               </div>
            </div>
          ...
          {sessionStatus === 'ended' && (
            <div className="fixed inset-0 z-[200] bg-black/90 backdrop-blur-md flex flex-col items-center justify-center text-center p-6 animate-in fade-in duration-500">
              ...
              <p className="text-gray-400 max-w-md mx-auto mb-8">
                The mentor has ended the session. You will be redirected to the dashboard in a few seconds.
              </p>
  ...
  function CustomParticipantTile(props: any) {
    ...
    if (!p) return <div className="bg-gray-900 w-full h-full animate-pulse flex items-center justify-center text-[10px] text-white/20 uppercase font-black">Connecting...</div>;

    return (
      <div className="relative w-full h-full group overflow-hidden">
        <ParticipantTile {...props} trackRef={trackRef} className="w-full h-full" />
...
function MediaControl({ source, onIcon, offIcon, disabled, minimal }: any) {
  ...
  return (
    ...
    >
      ...
      <span className={`text-[8px] md:text-[9px] font-black uppercase tracking-widest ${enabled ? 'text-gray-300' : 'text-red-500'}`}>
        {source === Track.Source.Microphone ? (enabled ? 'Mute' : 'Unmute') : (enabled ? 'Stop' : 'Start')}
      </span>
    </button>
  );
}

function ControlActionButton({ icon, label, onClick, isActive, activeColor = "text-white", minimal }: any) {
  if (minimal) {
    return (
      <button 
        onClick={onClick}
      ...
      <div className="py-2 max-h-64 overflow-y-auto">
        {devices.length === 0 ? (
          <div className="px-4 py-6 text-center">
            <p className="text-[10px] text-gray-500 font-bold mb-3">No devices found</p>
            <button 
              onClick={requestPermission}
