...
function MyVideoConference({
  ...
  const mainSpeakerTrack = useMemo(() => {
    ...
    devLog("Searching for mentor track. MentorId:", mentorId, "Tracks:", tracks.length);
    ...
    if (anyCamera) {
      devLog("Using secondary fallback camera:", anyCamera.participant.identity);
      return anyCamera;
    }
  ...
  const activeSpeakers = useMemo(() => {
    ...
    const speaking = nonLocal
      ...
      .sort((a, b) => (b.lastSpokeAt?.getTime() || 0) - (a.lastSpokeAt?.getTime() || 0));
    ...
    const recent = nonLocal
      .filter((p) => p.lastSpokeAt && !p.isSpeaking && !spokenIds.has(p.identity))
      .sort((a, b) => (b.lastSpokeAt!.getTime() || 0) - (a.lastSpokeAt!.getTime() || 0));
              ...
              >
                ...
                {gridParticipants.length > 4 && (
                  <div className="col-span-full flex justify-center py-2">
                    <button
                      onClick={() => { setSidebarTab("participants"); setShowSidebar(true); }}
  ...
  function ParticipantGridTile({ participant, trackRef }: { participant: any; trackRef?: any }) {
    return (
        ...
        {trackRef ? (
          ...
          <div className="w-full h-full bg-[#14141c] flex flex-col items-center justify-center gap-2 p-3">
            ...
            <div className="flex items-center gap-1.5">
              <div className={`w-1.5 h-1.5 rounded-full ${participant.isMicrophoneEnabled ? 'bg-green-500 shadow-[0_0_6px_rgba(34,197,94,0.5)]' : 'bg-red-500'}`} />
              <span className="text-[11px] font-bold text-white/60 truncate max-w-[120px]">
                {participant.name || participant.identity}
              </span>
            </div>
            {participant.isSpeaking && (
              <div className="text-[9px] font-bold text-green-400/70 uppercase tracking-widest">Speaking</div>
            )}
          </div>
        )}
      </div>
  ...
  function ParticipantAvatarTile({ participant, compact }: { participant: any; compact?: boolean }) {
    return (
      <div className="w-full h-full bg-[#14141c] flex flex-col items-center justify-center p-1">
        <div className={`rounded-full bg-white/5 border border-white/10 flex items-center justify-center ${compact ? 'w-6 h-6' : 'w-8 h-8'}`}>
          <User className={compact ? 'w-3.5 h-3.5 text-white/30' : 'w-4 h-4 text-white/30'} />
        </div>
        <span className="text-[8px] font-bold text-white/40 truncate max-w-[100px] mt-0.5 text-center leading-tight">
          {participant.name || participant.identity}
        </span>
        <div className="flex items-center gap-1 mt-0.5">
          <div className={`w-1 h-1 rounded-full ${participant.isMicrophoneEnabled ? 'bg-green-500' : 'bg-red-500'}`} />
          {participant.isSpeaking && <span className="text-[7px] font-bold text-green-400">SPK</span>}
        </div>
      </div>
    );
  }
