...
serve(async (req) => {
  ...
  try {
    ...
    if (egressEventTypes.includes(event.event)) {
      const egressInfo = (event as any).egressInfo || (event as any).egress_info
      const egressId = egressInfo?.egressId || egressInfo?.egress_id

      console.log(`Webhook egress event: ${event.event}, egress_id: ${egressId}`)
      ...
      if (egressId) {
        ...
        switch (event.event) {
          ...
          case 'egress_completed': {
            ...
            if (fileResults && fileResults.length > 0) {
              ...
              const filePath = fileResult?.filePath || fileResult?.file_path || fileResult?.location
              if (filePath) {
        ...
        if (Object.keys(updateData).length > 0) {
          ...
          if (error) {
            ...
            console.log(`Updated recording egress=${egressId}: ${JSON.stringify(updateData)}`)
          }
        }
      }
